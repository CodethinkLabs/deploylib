    ...
    def add_line(self, line):
        ...
        if self.lines_added == 0:
            if len(self.text) == 0 or self.text[-1] is not '\n':
                self.text += '\n'
            self.text += '# deploylib default btrfs system layout\n'
        self.lines_added += 1
...
class WriteExtension(object):
    ...
    def setup_logging(self):
        '''Direct all logging output to DEPLOY_LOG_FD, if set.

        This file descriptor can be read by a tool using deploylib
        and written into its own log file.

        '''
        log_write_fd = int(os.environ.get('DEPLOY_LOG_FD', 0))
    ...
    def mkfs_btrfs(self, location):
        ...
        try:
            ...
            if 'unrecognized option \'--features\'' in e.output:
                ...
                subprocess.check_call(['mkfs.btrfs', '-f', '-L', 'baserock',
                                       location])
            else:
                raise
